...
import { Brain, ChevronRight, Gauge, Plus, Sparkles, UserRound, Users } from 'lucide-react'
...
export default function OnboardingGate({ children }: { children: React.ReactNode }) {
  ...
  function createAndOpen(nextMode: StartMode, options?: { level?: 'A1' | 'A2' | 'B1'; placement?: PlacementEvidence }) {
    ...
    if (nextMode === 'zero') {
      ...
      const skillSeed = evidence.level === 'A2' ? 18 : 6
      saveProgress({
        ...defaultProgress,
        skillXp: { lesen: skillSeed, hören: Math.max(2, skillSeed - 3), schreiben: Math.max(2, skillSeed - 4), sprechen: 0, grammatik: skillSeed, wortschatz: skillSeed },
        learningItems: Object.fromEntries(evidence.knownTargets.map(key => [key, {
          key,
          kind: key.startsWith('grammar:') ? 'grammar' : key.startsWith('vocab:') ? 'vocabulary' : 'pattern',
  ...
  return <main className="min-h-screen bg-[#f7f8f4] px-4 py-8">
    <div className="mx-auto max-w-2xl">
      ...
      {step === 'start' && <div className="space-y-3"><h2 className="text-2xl font-black">Wie möchtest du starten?</h2><Choice icon={Sparkles} title="Ich fange bei null an" text="Keine Vorkenntnisse. Wir beginnen mit wenigen Wörtern und bauen alles Schritt für Schritt auf." onClick={() => createAndOpen('zero')}/><Choice icon={Brain} title="Ich kann schon etwas Slowenisch" text="Deine Selbsteinschätzung ist nur ein Startpunkt. Die App korrigiert das Lernmodell später anhand deiner Antworten." onClick={() => setStep('self-assessment')}/><Choice icon={Gauge} title="Mach einen Einstufungstest mit mir" text="Wir starten leicht und gehen nur bei ausreichender Evidenz zu schwierigeren Fragen." onClick={() => setStep('placement')}/></div>}
