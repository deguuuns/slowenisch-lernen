...
export default function SpeechPractice({ prompt, expected, onResult }: { prompt: string; expected: string; onResult?: (correct:boolean, actual:string) => void }) {
  ...
  function start() {
    ...
    r.onresult = (e:any) => { const t = e.results[0][0].transcript; setAnswer(t); setChecked(false) }
  ...
  const correct = checked && isEquivalent(answer, expected)
  return <div className="space-y-4">
    ...
    <div className="rounded-3xl bg-white p-5 shadow-soft">
      ...
      <div className="flex gap-2">
        <input value={answer} onChange={e => {setAnswer(e.target.value); setChecked(false)}} placeholder="Sprich oder tippe auf Slowenisch …" className="min-w-0 flex-1 rounded-2xl border border-slate-200 px-4 py-3 outline-none focus:border-lime-500"/>
      ...
      {checked && <div className={`mt-4 rounded-2xl p-4 ${correct ? 'bg-lime-50 text-lime-900' : 'bg-amber-50 text-amber-950'}`}>
        {correct ? <><strong>Odlično!</strong> Deine Antwort ist richtig.</> : <><strong>Noch nicht ganz.</strong><div className="mt-2">Korrektur: <span className="font-semibold">{expected}</span></div></>}
      </div>}
    </div>
  </div>
}
