...
export default function ListeningPractice({vocabulary,progress,onResult}:Props){
 ...
 <div className="mt-4 rounded-2xl bg-lime-50 p-4 text-sm text-lime-900"><b>Zuerst nur hören.</b> Text und Übersetzung sind Hilfen und werden beim Ergebnis als Hinweis berücksichtigt.</div>
 ...
 {showText&&<p className="mt-4 rounded-2xl bg-slate-50 p-4 text-lg leading-8">{item.text}</p>}{showTranslation&&<p className="mt-3 text-sm leading-6 text-slate-500">{item.translation}</p>}
 ...
 <div className="mt-5 flex justify-end"><button className="btn-primary" onClick={next}>Nächste Hörübung</button></div></div>
}
