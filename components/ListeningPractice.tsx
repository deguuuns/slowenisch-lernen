...
export default function ListeningPractice({vocabulary,progress,onResult,onComplete,focusMode=true}:Props){
 ...
 const [itemIndex,setItemIndex]=useState(0),[questionIndex,setQuestionIndex]=useState(0),[showText,setShowText]=useState(false),[showTranslation,setShowTranslation]=useState(false),[answerValue,setAnswerValue]=useState<string|null>(null),[checked,setChecked]=useState(false)
 ...
 function choose(option:string){if(checked||!question)return;setAnswerValue(option)}
 function submit(){if(!answerValue||checked||!question)return;setChecked(true);const isCorrect=answerValue===question.answer;const exercise:Exercise={id:`listening:${item.id}:${questionIndex}`,lesson:1,type:'choice',prompt:question.question,answer:question.answer,evaluationMode:'exact',skillTargets:['listening'],targetContentKeys:item.targetKeys,learningPhase:'recognize'};onResult(exercise,isCorrect,{responseMs:Math.max(250,Date.now()-startedAt.current),hintsUsed:(showText?1:0)+(showTranslation?1:0)})}
 function resetQuestion(){setAnswerValue(null);setChecked(false);setShowText(false);setShowTranslation(false);startedAt.current=Date.now()}
 function advance(){if(questionIndex<item.prompts.length-1){setQuestionIndex(index=>index+1);resetQuestion();return}if(onComplete){onComplete();return}setItemIndex(index=>(index+1)%Math.max(1,ranked.length));setQuestionIndex(0);resetQuestion()}
 ...
 const body=<div className="exercise-shell">
   <div><div className="flex items-center justify-between gap-3 text-[11px] font-bold"><span className="inline-flex items-center gap-1.5 text-lime-700"><Headphones size={13}/>Hören</span><span className="text-slate-400">{questionIndex+1}/{item.prompts.length}</span></div><div className="progress-track mt-1.5"><div className="progress-fill" style={{width:`${Math.round(((questionIndex+(checked?1:0))/Math.max(1,item.prompts.length))*100)}%`}}/></div></div>
   {!checked?<div className="exercise-content">
     ...
     <div className="exercise-choice-grid mt-2">{question.options.map(option=>{const selected=answerValue===option;return <button key={option} type="button" aria-pressed={selected} onClick={()=>choose(option)} className={`exercise-choice surface-interactive w-full border text-left font-bold ${selected?'border-lime-500 bg-lime-50 ring-2 ring-lime-100':'border-slate-200 bg-white'}`}>{option}</button>})}</div>
     <button className="btn-primary mt-3 w-full" disabled={!answerValue} onClick={submit}>Prüfen</button>
   ...
   </div>:<div className="exercise-content exercise-feedback" role="status" aria-live="polite">{correct?<CheckCircle2 className="mx-auto text-lime-700" size={38}/>:<XCircle className="mx-auto text-amber-700" size={38}/>}<div className="mt-2 text-2xl font-black">{correct?'Richtig':'Noch nicht'}</div>{!correct&&<div className="mt-3 text-base font-bold">Richtig: {question.answer}</div>}<button className="btn-primary mt-3 w-full" onClick={advance}>Weiter</button></div>}
 </div>
