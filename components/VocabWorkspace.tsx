...
function VocabularyTest({words,progress,onResult,onExit}:{words:Vocabulary[];progress:UserProgress;onResult:Props['onResult'];onExit:()=>void}){
  ...
  if(!word)return <LearningFocusPortal><div className="exercise-shell"><div/><div className="exercise-feedback"><h2 className="text-xl font-black">Noch keine Vokabeln für einen Test</h2><p className="mt-2 text-sm text-slate-500">Lerne zuerst einige Wörter.</p></div><div><button className="btn-primary w-full" onClick={onExit}>Zurück</button></div></div></LearningFocusPortal>
...
function ConjugationPractice({progress,onResult,onExit}:{progress:UserProgress;onResult:Props['onResult'];onExit:()=>void}){
  ...
  const startedAt=useRef(Date.now())
  if(!candidates.length)return <LearningFocusPortal><div className="exercise-shell"><div/><div className="exercise-feedback"><h2 className="text-xl font-black">Noch keine Verbformen freigeschaltet</h2><p className="mt-2 text-sm text-slate-500">Die Formen werden im Curriculum Schritt für Schritt eingeführt.</p></div><div><button className="btn-primary w-full" onClick={onExit}>Zurück</button></div></div></LearningFocusPortal>
