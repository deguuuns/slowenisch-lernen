...
export default function InputPractice({vocabulary,progress,onResult}:Props){
 const [section,setSection]=useState<'listening'|'stories'>('listening')
 ...
 function answer(qi:number,option:string){if(answers[qi])return;const q=s.questions[qi];const correct=option===q.answer;setAnswers(a=>({...a,[qi]:option}));const exercise:Exercise={id:`input:${s.id}:${qi}`,lesson:1,type:'choice',prompt:q.prompt,answer:q.answer,evaluationMode:'exact',skillTargets:showText?['recognition']:['listening'],targetContentKeys:[`skill:${showText?'recognition':'listening'}`]};onResult(exercise,correct,{responseMs:Math.max(250,Date.now()-startedAt.current),hintsUsed:showTranslation?1:0})}
 ...
 return <div className="min-w-0 space-y-4"><div className="card"><div className="text-xs font-bold uppercase tracking-wide text-lime-700">Verstehen</div><h3 className="text-xl font-black">Hören & Geschichten</h3><p className="mt-1 text-sm text-slate-600">Trainiere zuerst gezielt Wort → Satz → Dialog → Geschichte oder arbeite mit längeren verständlichen Mini-Texten.</p><div className="mt-3 flex flex-wrap gap-2"><button onClick={()=>setSection('listening')} className={`rounded-full px-3 py-2 text-sm font-bold ${section==='listening'?'bg-slate-950 text-white':'bg-slate-100'}`}>Systematisches Hörtraining</button><button onClick={()=>setSection('stories')} className={`rounded-full px-3 py-2 text-sm font-bold ${section==='stories'?'bg-slate-950 text-white':'bg-slate-100'}`}>Mini-Geschichten</button></div></div>
 {section==='listening'?<ListeningPractice vocabulary={vocabulary} progress={progress} onResult={onResult}/>:<div className="card min-w-0"><div className="flex items-start gap-3"><div className="min-w-0 flex-1"><div className="text-xs font-bold uppercase tracking-wide text-lime-700">Verstehen · {s.level}</div><h3 className="text-xl font-black">{s.title}</h3><div className="mt-1 text-sm text-slate-500">{Math.round(current.ratio*100)} % der Zielwörter bereits bekannt · nach deinem Lernstand ausgewählt</div></div><AudioButton text={s.text}/></div>
 ...
 <div className="mt-5 flex justify-end"><button className="btn-primary" onClick={next}>Nächster Text</button></div></div>}</div>
}
