...
function StoryFocus({ranked,onResult,onExit}:{ranked:{story:(typeof INPUT_STORIES)[number];ratio:number}[];onResult:Props['onResult'];onExit:()=>void}){
 const [storyIndex,setStoryIndex]=useState(0),[questionIndex,setQuestionIndex]=useState(0),[overlay,setOverlay]=useState<'text'|'translation'|null>(null),[usedText,setUsedText]=useState(false),[usedTranslation,setUsedTranslation]=useState(false),[answerValue,setAnswerValue]=useState<string|null>(null)
 ...
 function next(){if(questionIndex<s.questions.length-1){setQuestionIndex(index=>index+1);setAnswerValue(null);setOverlay(null);setUsedText(false);setUsedTranslation(false);startedAt.current=Date.now();return}setStoryIndex(index=>(index+1)%Math.max(1,ranked.length));setQuestionIndex(0);setAnswerValue(null);setOverlay(null);setUsedText(false);setUsedTranslation(false);startedAt.current=Date.now()}
 ...
 const body=<div className="exercise-shell"><div className="flex items-center justify-between gap-3 text-[11px] font-bold"><button onClick={onExit} className="btn-quiet -ml-2">Zurück</button><span className="text-slate-400">Frage {questionIndex+1}/{s.questions.length}</span></div>{!answerValue?<div className="exercise-content"><div className="flex items-center justify-between gap-3"><div className="min-w-0"><div className="eyebrow">{s.level} · {Math.round(current.ratio*100)}% bekannt</div><h2 className="mt-1 text-lg font-black leading-tight">{s.title}</h2></div><AudioButton text={s.text} compact/></div><div className="mt-2 flex gap-1.5"><button className="btn-quiet flex-1" onClick={openText}>Text</button><button className="btn-quiet flex-1" onClick={openTranslation}>Deutsch</button></div><div className="mt-3 text-lg font-black leading-tight">{q.prompt}</div><div className="exercise-choice-grid mt-2">{q.options.map(option=><button key={option} onClick={()=>answer(option)} className="exercise-choice surface-interactive border border-slate-200 bg-white text-left font-bold">{option}</button>)}</div></div>:<div className="exercise-content exercise-feedback" role="status" aria-live="polite">{correct?<CheckCircle2 className="mx-auto text-lime-700" size={38}/>:<XCircle className="mx-auto text-amber-700" size={38}/>}<div className="mt-2 text-2xl font-black">{correct?'Richtig':'Noch nicht'}</div>{!correct&&<div className="mt-3 text-base font-bold">Richtig: {q.answer}</div>}</div>}<div className="exercise-actions">{answerValue&&<button className="btn-primary w-full" onClick={next}>Weiter</button>}</div>{overlay&&<div className="exercise-overlay" role="dialog" aria-label={overlay==='text'?'Geschichtentext':'Deutsche Übersetzung'}><div className="eyebrow">{overlay==='text'?'Geschichtentext':'Übersetzung'}</div><div className="mt-3 text-base font-semibold leading-7">{overlay==='text'?s.text:s.translation}</div><button className="btn-primary mt-5 w-full" onClick={()=>setOverlay(null)}>Zur Frage</button></div>}</div>
