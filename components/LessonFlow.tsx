...
export default function LessonFlow({lessonId,progress,setProgress,onExerciseResult,onFinish,focusMode=true}:{lessonId:number;progress:UserProgress;setProgress:Dispatch<SetStateAction<UserProgress>>;onExerciseResult:(exercise:Exercise,correct:boolean,meta:ExerciseResultMeta)=>void;onFinish:()=>void;focusMode?:boolean}){
  ...
  function finishCheckpoint(){recordCurrentExam('checkpoint');setActiveSession(null);if(blockIndex<blocks.length-1){setBlockIndex(index=>index+1);setWordIndex(0);setVerbIndex(0);setGrammarIndex(0);setStage('word-intro')}else setStage('final-exam')}
  ...
  if(stage==='word-intro'&&block){const word=block.words[wordIndex];active=<LessonStep><div className="flex items-center justify-between gap-3 text-[11px] font-bold"><span className="text-lime-700">Verstehen</span><span className="text-slate-400">Wort {wordIndex+1}/{block.words.length}</span></div><div className="mt-4 flex items-start justify-between gap-4"><div className="min-w-0"><h2 className="break-words text-4xl font-black tracking-tight [overflow-wrap:anywhere]">{word.sl}</h2><div className="mt-1 text-lg font-semibold text-slate-500">{word.de}</div></div><AudioButton text={word.sl} compact/></div><div className="mt-4 rounded-xl bg-slate-50 p-3"><div className="text-sm font-bold leading-snug">{word.example}</div><div className="mt-1 text-xs leading-snug text-slate-500">{word.exampleDe}</div></div><button onClick={nextWord} className="btn-primary mt-4 w-full">{wordIndex===block.words.length-1?'Weiter':'Nächstes Wort'} <ChevronRight size={18}/></button></LessonStep>}
  if(stage==='verb-intro'&&currentVerb) active=<LessonStep><div className="eyebrow">Verb verstehen</div><div className="mt-3 flex items-start justify-between gap-3"><div className="min-w-0"><h2 className="break-words text-3xl font-black">{currentVerb.infinitiveSl}</h2><div className="text-sm text-slate-500">{currentVerb.infinitiveDe}</div></div><AudioButton text={currentVerb.infinitiveSl} compact/></div><div className="mt-4 grid gap-1.5">{currentVerb.forms.slice(0,3).map(form=><div key={form.person} className="flex items-center justify-between gap-3 rounded-xl bg-slate-50 px-3 py-2"><b className="text-sm">{form.pronounSl} {form.formSl}</b><span className="truncate text-xs text-slate-500">{form.translationDe}</span></div>)}</div><button onClick={unlockCurrentVerb} className="btn-primary mt-4 w-full">Jetzt bilden <ChevronRight size={18}/></button></LessonStep>
  ...
  if(stage==='block-practice'&&activeSession) active=activeSession.exercises.length?<ExerciseDeck key={activeSession.sessionId} session={activeSession} onResult={onExerciseResult} onComplete={finishBlockPractice} focusMode={false}/>:<SkipStep title="Keine weitere Übung nötig" onNext={finishBlockPractice}/>
  if(stage==='checkpoint'&&activeSession) active=activeSession.exercises.length?<ExerciseDeck key={activeSession.sessionId} session={activeSession} onResult={onExerciseResult} onComplete={finishCheckpoint} focusMode={false}/>:<SkipStep title="Zwischenprüfung übersprungen" onNext={finishCheckpoint}/>
  ...
  if(stage==='done') active=<LessonStep center><CheckCircle2 className="mx-auto text-lime-700" size={40}/><div className="eyebrow mt-4">100%</div><h2 className="mt-1 text-3xl font-black">Lektion geschafft</h2><p className="mt-2 text-sm text-slate-500">Unsichere Inhalte werden gezielt wiederholt.</p><button onClick={onFinish} className="btn-primary mt-5 w-full">Lektion abschließen</button></LessonStep>
