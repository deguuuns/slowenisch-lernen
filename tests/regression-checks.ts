...
// Answer integrity: every curated exercise is internally consistent.
...
// Normal sentence exercises remain locked until required verb forms are actually trained.
...
const due=fresh({preferences:{...defaultProgress.preferences,onboardingCompleted:true},introducedWords:['v001'],introducedGrammarRules:['greeting-basic'],reviews:[{key:'vocab:v001',status:'unsicher',intervalIndex:0,dueAt:0,updatedAt:1}]});assert.equal(buildAdaptiveRecommendation(due,enriched,vocabulary,1,now).kind,'review')
...
// Dedicated verb module: full singular intro, German translations, three rounds per person, active mastery gate.
...
// Exam rotation preserves complete exercise objects and remains diverse.
