...
assert.doesNotMatch(css, /\.daily-training-screen\s*\{[^}]*position:\s*fixed/s, 'Daily training must not be locked into a fixed viewport')
assert.doesNotMatch(css, /\.exercise-content\s*\{[^}]*overflow-y:\s*auto/s, 'Exercises should use normal document flow instead of nested vertical scrolling')
assert.doesNotMatch(css, /\.daily-training-content\s*\{[^}]*overflow:\s*hidden/s, 'Daily training content must never be clipped')
