...
function Header({streak}:{streak:number}) { return <header className="flex items-center justify-between"><div><div className="text-xs font-bold uppercase tracking-[0.24em] text-lime-700">Slovensko</div><h1 className="text-2xl font-black tracking-tight">Slowenisch, das du wirklich sprichst.</h1></div><div className="flex items-center gap-2 rounded-full bg-white px-3 py-2 shadow-soft"><Flame size={18}/><span className="font-bold">{streak}</span><span className="hidden text-sm text-slate-500 sm:inline">Tage</span></div></header> }
...
function HomeView({progress,activeLesson,onContinue,onGo}:{progress:UserProgress;activeLesson:number;onContinue:()=>void;onGo:(t:Tab)=>void}) {
  ...
  return <div className="space-y-6">
    <div className="overflow-hidden rounded-[2rem] bg-slate-950 p-6 text-white shadow-soft md:p-8"><div className="max-w-2xl"><div className="mb-2 text-sm font-semibold text-lime-300">Dobrodošel!</div><h2 className="text-3xl font-black md:text-5xl">Heute nicht nur lernen. <span className="text-lime-300">Heute sprechen.</span></h2><p className="mt-4 max-w-xl text-slate-300">Neue Wörter kommen in kleinen Portionen und werden sofort aktiv benutzt.</p><button onClick={onContinue} className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-lime-300 px-5 py-3 font-black text-slate-950">Heute weiterlernen <ChevronRight/></button></div></div>
    <div className="grid gap-3 sm:grid-cols-3"><Metric icon={Brain} value={`${progress.wordsLearned.length} / 1000`} label="Aktiver Wortschatz"/><Metric icon={RotateCcw} value={String(due)} label="Heute wiederholen"/><Metric icon={GraduationCap} value={`${progress.completedLessons.length} / 30`} label="Lektionen"/></div>
    <div className="grid gap-4 lg:grid-cols-2"><div className="card"><div className="text-sm font-bold text-lime-700">Als Nächstes</div><h3 className="mt-1 text-xl font-black">Lektion {activeLesson}: {lessons[activeLesson-1]?.title}</h3><p className="mt-2 text-slate-600">{lessons[activeLesson-1]?.subtitle}</p></div><div className="card"><div className="flex items-center gap-2"><Sparkles size={18}/><h3 className="font-black">Dein Muster für heute</h3></div><div className="mt-4 rounded-2xl bg-lime-50 p-4"><div className="text-sm text-slate-500">KJE → WO?</div><div className="mt-1 text-xl font-bold">Sem v Sloveniji.</div><div className="mt-3 text-sm text-slate-500">KAM → WOHIN?</div><div className="mt-1 text-xl font-bold">Grem v Slovenijo.</div></div></div></div>
    <div className="grid grid-cols-2 gap-3 sm:grid-cols-4"><Quick icon={RotateCcw} label="Wiederholen" onClick={()=>onGo('review')}/><Quick icon={Mic2} label="Sprechen" onClick={()=>onGo('speak')}/><Quick icon={Brain} label="Vokabeln" onClick={()=>onGo('vocab')}/><Quick icon={Trophy} label="Fortschritt" onClick={()=>onGo('progress')}/></div>
  </div>
}
...
function LessonArea({lessonId,setLessonId,progress,setProgress,finishLesson}:{lessonId:number;setLessonId:(id:number)=>void;progress:UserProgress;setProgress:(f:any)=>void;finishLesson:(id:number)=>void}) {
  return <div className="space-y-4"><div className="flex gap-2 overflow-x-auto pb-1">{lessons.map(l=><button key={l.id} onClick={()=>setLessonId(l.id)} className={`shrink-0 rounded-full px-4 py-2 text-sm font-bold ${lessonId===l.id?'bg-slate-950 text-white':'bg-white text-slate-600'}`}>Lektion {l.id}{progress.completedLessons.includes(l.id)?' ✓':''}</button>)}</div><LessonFlow key={lessonId} lessonId={lessonId} progress={progress} setProgress={setProgress} onExerciseResult={(e,c)=>handleExercise(e,c,setProgress)} onFinish={()=>finishLesson(lessonId)}/></div>
}

function ConversationCard({lesson}:{lesson:number}) { const c=conversations.find(c=>c.lesson===lesson)!; return <div className="card"><div className="mb-4 flex items-center gap-2"><MessageCircle size={20}/><h3 className="text-xl font-black">{c.title}</h3></div><div className="space-y-3">{c.turns.map((t,i)=><div key={i} className={`max-w-[90%] rounded-2xl p-3 ${t.speaker==='Tutor'?'bg-slate-100':'ml-auto bg-lime-100'}`}><div className="mb-1 text-xs font-bold uppercase text-slate-400">{t.speaker}</div><div className="flex items-center gap-2"><span className="font-semibold">{t.sl}</span><AudioButton text={t.sl} compact/></div>{t.de&&<div className="mt-1 text-sm text-slate-500">{t.de}</div>}</div>)}</div></div> }
...
function ReviewView({progress,setProgress}:{progress:UserProgress;setProgress:(f:any)=>void}) {
  ...
  return <div className="space-y-5"><div className="card"><h2 className="text-3xl font-black">Wiederholen</h2><p className="mt-2 text-slate-600">Fehlerhafte Inhalte kommen früher wieder. Richtige Antworten bekommen größere Abstände.</p></div><ExerciseDeck exercises={selected} onResult={(e,c)=>handleExercise(e,c,setProgress)}/></div>
}
...
function SpeakView({setProgress}:{setProgress:(f:any)=>void}) {
  ...
  return <div className="space-y-5"><div className="card"><h2 className="text-3xl font-black">Sprechen</h2><p className="mt-2 text-slate-600">Hören → selbst antworten → korrigieren.</p></div><SpeechPractice prompt={prompts[idx][0]} expected={prompts[idx][1]} onResult={(correct)=>{setProgress((p:UserProgress)=>({...p,speakingMinutes:+(p.speakingMinutes+0.2).toFixed(1)}));if(correct)setTimeout(()=>setIdx((idx+1)%prompts.length),900)}}/><TutorChat/></div>
}
