...
export default function Page() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setSyncState(`synchronisiert · ${new Date().toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}`)
      } catch (error) {
  ...
  function finishLesson(id: number) {
    setProgress(previous => ({ ...previous, completedLessons: Array.from(new Set([...previous.completedLessons, id])), updatedAt: Date.now() }))
    setTab('progress')
  }
  ...
  return <main className="min-h-screen min-w-0 max-w-full overflow-x-hidden pb-[calc(6.5rem+env(safe-area-inset-bottom))] md:pb-8">
...
function Header({ streak, session, syncState, preferences, onSession, onPreferences }: { streak:number; session:CloudSession|null; syncState:string; preferences:LearnerPreferences; onSession:(session:CloudSession|null)=>void; onPreferences:(preferences:LearnerPreferences)=>void }) {
  return <header className="flex min-w-0 items-start justify-between gap-2 sm:items-center sm:gap-3"><div className="min-w-0 flex-1"><div className="text-xs font-bold uppercase tracking-[0.24em] text-lime-700">Slovensko</div><h1 className="min-w-0 break-words text-xl font-black tracking-tight sm:text-2xl [overflow-wrap:anywhere]">Slowenisch, das du wirklich sprichst.</h1></div><div className="flex shrink-0 items-center gap-2"><div className="hidden items-center gap-2 rounded-full bg-white px-3 py-2 shadow-soft sm:flex"><Flame size={18} /><span className="font-bold">{streak}</span><span className="text-sm text-slate-500">Tage</span></div><AccountMenu session={session} syncState={syncState} preferences={preferences} onSession={onSession} onPreferences={onPreferences} /></div></header>
}
...
function HomeView({ progress, activeLesson, onContinue, onGo }: { progress:UserProgress; activeLesson:number; onContinue:()=>void; onGo:(tab:Tab)=>void }) {
  ...
  return <div className="min-w-0 space-y-6"><div className="min-w-0 overflow-hidden rounded-[2rem] bg-slate-950 p-5 text-white shadow-soft sm:p-6 md:p-8"><div className="min-w-0 max-w-2xl"><div className="mb-2 text-sm font-semibold text-lime-300">Dein adaptiver Lernplan</div><h2 className="break-words text-2xl font-black sm:text-3xl md:text-5xl [overflow-wrap:anywhere]">Heute genau das, <span className="text-lime-300">was dir am meisten bringt.</span></h2><p className="mt-4 max-w-xl break-words text-slate-300">Fälligkeit, Antworttempo, Hilfen und aktive Produktion bestimmen die Auswahl – nicht künstliche Wiederholungen.</p><button onClick={onContinue} className="mt-6 inline-flex min-h-11 items-center gap-2 whitespace-normal rounded-2xl bg-lime-300 px-5 py-3 font-black text-slate-950">Empfehlung starten <ChevronRight /></button></div></div><div className="card min-w-0 border-2 border-lime-200"><div className="flex items-center gap-2 text-sm font-bold text-lime-700"><Sparkles size={18} />Heute empfohlen</div><h3 className="mt-2 break-words text-2xl font-black">{recommendation.title}</h3><p className="mt-2 break-words text-slate-600">{recommendation.reason}</p><div className="mt-4 break-words text-xs font-semibold uppercase tracking-wide text-slate-400">{plan.total ? `ca. ${plan.total} fällige Aufgaben` : 'keine reguläre Wiederholung fällig'} · Tagesziel {progress.preferences.dailyGoalMinutes} min</div></div><div className="grid min-w-0 gap-3 sm:grid-cols-4"><Metric icon={Brain} value={`${progress.wordsLearned.length} / 120`} label="Gelernter Wortschatz" /><Metric icon={RotateCcw} value={String(due)} label="Heute fällig" /><Metric icon={Sparkles} value={String(weak)} label="Schwache Bereiche" /><Metric icon={GraduationCap} value={`${progress.completedLessons.length} / 5`} label="Lektionen" /></div><div className="grid min-w-0 grid-cols-2 gap-3 sm:grid-cols-4"><Quick icon={RotateCcw} label="Wiederholen" onClick={() => onGo('review')} /><Quick icon={Mic2} label="Sprechen" onClick={() => onGo('speak')} /><Quick icon={Brain} label="Vokabeln" onClick={() => onGo('vocab')} /><Quick icon={Trophy} label="Fortschritt" onClick={() => onGo('progress')} /></div></div>
}

function Metric({ icon:Icon, value, label }: { icon:LucideIcon; value:string; label:string }) { return <div className="card min-w-0"><Icon className="mb-3" size={20} /><div className="break-words text-2xl font-black">{value}</div><div className="break-words text-sm text-slate-500">{label}</div></div> }
function Quick({ icon:Icon, label, onClick }: { icon:LucideIcon; label:string; onClick:()=>void }) { return <button onClick={onClick} className="card flex min-h-28 min-w-0 flex-col items-start justify-between text-left"><Icon /><span className="break-words font-bold">{label}</span></button> }
...
function ReviewView({ progress, activeLesson, setProgress }: { progress:UserProgress; activeLesson:number; setProgress:ProgressSetter }) {
  ...
  return <div className="min-w-0 space-y-5"><div className="card min-w-0"><div className="text-sm font-bold text-lime-700">Karteikasten</div><h2 className="mt-1 break-words text-2xl font-black sm:text-3xl">Nur wirklich fällige Karten</h2><p className="mt-2 break-words text-slate-600">{plan.total ? `${plan.total} Lernziel${plan.total === 1 ? '' : 'e'} sind jetzt fällig.` : 'Aktuell ist keine reguläre Karte fällig. Sichere Inhalte bleiben bis zu ihrem Termin im Kasten.'}</p></div>{reviewSession.exercises.length ? <ExerciseDeck key={reviewSession.sessionId} session={reviewSession} onResult={(exercise, correct, meta) => handleExercise(exercise, correct, meta, setProgress)} /> : <div className="card min-w-0"><b>Nichts zu wiederholen.</b><p className="mt-2 text-sm text-slate-600">Du kannst stattdessen mit neuem Stoff weitermachen.</p></div>}</div>
}
...
function handleExercise(exercise:Exercise, correct:boolean, meta:ExerciseResultMeta, setProgress:ProgressSetter) {
  setProgress(previous => {
    ...
    const now = Date.now()
    const reviews = scheduleExerciseReviews(previous.reviews, exercise, correct, now)
