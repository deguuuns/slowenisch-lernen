...
function weakItems(progress:UserProgress){return Object.values(progress.mastery||{}).filter(m=>m.attempts>=MIN_ATTEMPTS_FOR_WEAK&&m.score<WEAK_THRESHOLD).sort((a,b)=>a.score-b.score||b.attempts-a.attempts)}
function exercisesForMastery(items:MasteryItem[],exercises:Exercise[]){const keys=new Set(items.map(x=>x.key));return exercises.filter(ex=>{const exKeys=[...(ex.vocabularyIds||[]).map(id=>`vocab:${id}`),...(ex.grammarRuleIds||[]).map(id=>`grammar:${id}`)];if(ex.type==='choice')exKeys.push('skill:recognition');else if(ex.type==='translate-de-sl'||ex.type==='free')exKeys.push('skill:production');else exKeys.push('skill:grammar-application');return exKeys.some(k=>keys.has(k))})}
...
export function buildAdaptiveRecommendation(progress:UserProgress,rawExercises:Exercise[],vocabulary:Vocabulary[],activeLesson:number,now=Date.now()):AdaptiveRecommendation{
 ...
 if(viableTransfer)return {kind:'strengthen',title:'Grammatik auf neue Beispiele übertragen',reason:'Nach einem Grammatikfehler prüft die App mit einem anderen Satz, ob du die Regel wirklich verstanden hast.',priority:90,focusKeys:[`grammar:${viableTransfer.grammarRuleId}`],exerciseIds:[]}
 ...
 const budget=newWordBudget(progress),lessonWords=vocabulary.filter(v=>v.lesson===activeLesson),unseen=lessonWords.filter(v=>!progress.introducedWords.includes(v.id));return {kind:'new-content',title:`Weiter mit Lektion ${activeLesson}`,reason:unseen.length?`${Math.min(budget,unseen.length)} neue Wörter sind für den nächsten Lernblock vorgesehen (${progress.preferences?.pace||'normal'}es Tempo).`:'Die neuen Wörter sind eingeführt. Jetzt festigst du sie im Lektionenablauf.',priority:50,lessonId:activeLesson,focusKeys:unseen.slice(0,budget).map(v=>`vocab:${v.id}`),exerciseIds:exercises.filter(e=>e.lesson===activeLesson).map(e=>e.id).slice(0,8)}
}
...
export function buildAdaptiveReviewDeck(progress:UserProgress,rawExercises:Exercise[],limit=10,now=Date.now()):Exercise[]{
 ...
 const transfer=injectDueTransfer(chosen,progress.transferQueue||[],exercises,progress.recentAttempts?.length||0);return transfer.exercises.slice(0,limit)
}
