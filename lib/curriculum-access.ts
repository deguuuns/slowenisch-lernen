...
export function singularVerbIntroForVocabulary(ids:string[]){const verbIds=Array.from(new Set(ids.map(id=>VERB_FORMS[id]?.verbId).filter(Boolean))) as string[];return verbIds.map(verbId=>{const def=VERB_INTROS[verbId];if(!def)return null;return {...def,title:`${def.infinitiveSl} – ${def.infinitiveDe}`,keys:def.forms.map(f=>verbFormKey({verbId,person:f.person,number:'singular'}))}}).filter((v):v is NonNullable<typeof v>=>Boolean(v))}
