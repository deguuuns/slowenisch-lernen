...
export const LISTENING_CURRICULUM:ListeningItem[]=[
 ...
 {id:'listen-dialogue-restaurant',stage:'dialogue',title:'Im Restaurant',text:'Dober dan. Kaj želite piti? Želim vodo, prosim. Seveda. Želite še kaj? Ne, hvala.',translation:'Guten Tag. Was möchten Sie trinken? Ich möchte Wasser, bitte. Natürlich. Möchten Sie noch etwas? Nein, danke.',requiredWords:['dober dan','želeti','piti','voda','prosim','hvala'],recommendedSpeed:'normal',targetKeys:['skill:listening','topic:restaurant'],prompts:[{question:'Kaj želi gost piti?',options:['Vodo','Kavo','Čaj'],answer:'Vodo'},{question:'Ali želi še kaj?',options:['Ne','Da','Ne vemo'],answer:'Ne'}]},
 {id:'listen-dialogue-travel',stage:'dialogue',title:'Unterwegs',text:'Kam greš danes? Grem v Ljubljano. Kako greš? Peljem se z avtom.',translation:'Wohin gehst/fährst du heute? Ich fahre nach Ljubljana. Wie fährst du? Ich fahre mit dem Auto.',requiredWords:['kam','danes','iti','Ljubljana','peljati se','avto'],recommendedSpeed:'normal',targetKeys:['skill:listening','topic:travel','grammar:case:accusative'],prompts:[{question:'Kam gre oseba?',options:['V Ljubljano','Domov','V Maribor'],answer:'V Ljubljano'},{question:'Kako gre?',options:['Z avtom','Z vlakom','Peš'],answer:'Z avtom'}]},
 {id:'listen-story-day',stage:'story',title:'Moj dan',text:'Zjutraj pijem kavo in potem grem v službo. Popoldne grem v trgovino. Zvečer sem doma in kuham večerjo.',translation:'Morgens trinke ich Kaffee und gehe danach zur Arbeit. Nachmittags gehe ich einkaufen. Abends bin ich zu Hause und koche Abendessen.',requiredWords:['zjutraj','piti','kava','služba','popoldne','trgovina','zvečer','doma','kuhati'],recommendedSpeed:'normal',targetKeys:['skill:listening','topic:daily-routine'],prompts:[{question:'Kam grem zjutraj?',options:['V službo','Domov','V hotel'],answer:'V službo'},{question:'Kje sem zvečer?',options:['Doma','V trgovini','V službi'],answer:'Doma'}]},
]
...
export function listeningKnownRatio(item:ListeningItem,knownWords:Set<string>){if(!item.requiredWords.length)return 1;const known=new Set([...knownWords].map(x=>x.toLocaleLowerCase('sl-SI')));return item.requiredWords.filter(word=>known.has(word.toLocaleLowerCase('sl-SI'))).length/item.requiredWords.length}
