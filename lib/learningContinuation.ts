...
export function resolveLearningContinuation(progress: UserProgress, completedPhase: number | null, now = Date.now()): LearningContinuation {
  if (isBeginnerFoundationComplete(progress)) {
    return {
      type: 'next-course-stage', phase: null, title: 'A1-Aufbau',
  ...
  if (completedPhase !== null && current.id > completedPhase) {
    return {
      type: 'advance-curriculum', fromPhase: completedPhase, toPhase: current.id,
      title: current.title, message: `Sehr gut. Als Nächstes: ${current.title}.`,
    }
  }

  return {
    type: 'continue-current-phase', phase: current.id, title: current.title,
...
export function phaseTitle(phase: number | null) {
  ...
  return beginnerCurriculum.find(item => item.id === phase)?.title ?? (phase === 11 ? 'A1-Aufbau' : 'Nächstes Lernziel')
}
