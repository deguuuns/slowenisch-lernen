...
/**
 * Canonical morphology source for Slovene verbs used by lessons, vocabulary detail,
 * conjugation practice and curriculum gating. UI code must not define parallel verb tables.
 */
...
export const vocabularyVerbFormRequirements: Record<string, VerbFormRequirement> = {
  ...
  v107:{verbId:'želeti',person:1,number:'singular'},
}
...
export function verbFormFor(requirement: VerbFormRequirement) {
  return verbById(requirement.verbId)?.forms.find(form => form.person === requirement.person && form.number === requirement.number)
}

export function verbRequirementForVocabularyId(id:string) {
  return vocabularyVerbFormRequirements[id]
}

export function isRegisteredVerbFormVocabularyId(id:string) {
  return Boolean(vocabularyVerbFormRequirements[id])
}
...
export function singularVerbIntroFromRegistry(verbId:string) {
  ...
  return {
    ...
    forms:verb.forms
      .filter(form=>form.number==='singular')
      .map(form=>({person:form.person,pronounSl:form.pronoun,formSl:form.form,translationDe:form.translation})),
  }
}
