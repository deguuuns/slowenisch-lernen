...
const base:UserProgress={completedLessons:[],streak:1,introducedWords:[],wordsLearned:[],secureWords:[],mistakes:[],reviews:[],speakingMinutes:0,listeningMinutes:0,mastery:{},recentAttempts:[],transferQueue:[]}
...
const ex:Exercise[]=[{id:'e1',lesson:1,type:'translate-de-sl',prompt:'Ich habe zwei Brüder.',answer:'Imam dva brata.',vocabularyIds:['v1'],grammarRuleIds:['dual-masculine']},{id:'e2',lesson:1,type:'translate-de-sl',prompt:'Ich habe zwei Söhne.',answer:'Imam dva sina.',grammarRuleIds:['dual-masculine']},{id:'e3',lesson:1,type:'choice',prompt:'Bruder',answer:'brat',alternatives:['sestra'],vocabularyIds:['v1']}]
...
export function runAdaptiveCurriculumChecks(){
 const due={...base,reviews:[{key:'e1',status:'unsicher' as const,dueAt:0,intervalIndex:0}]};assert(buildAdaptiveRecommendation(due,ex,vocab,1,1).kind==='review','due reviews must win')
 ...
 const transfer={...base,recentAttempts:[1,2,3,4].map(n=>({exerciseId:'e1',correct:true,responseMs:5000,hintsUsed:0,occurredAt:n})),transferQueue:[{sourceExerciseId:'e1',grammarRuleId:'dual-masculine',dueAfter:3,createdAt:1}]};assert(buildAdaptiveRecommendation(transfer,ex,vocab,1,1).focusKeys.includes('grammar:transfer'),'due grammar transfer should outrank new content');assert(buildAdaptiveReviewDeck(transfer,ex,10,1).some(x=>x.id==='e2'),'transfer must use a different exercise with same grammar rule')
